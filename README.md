...
### Primary objective: multi-seed `leo_fast_ho` (LeoAware v3.1)

Seeds 13,7,42,99,123. Constraint: Leo p95 mean ≤ BBR p95 mean.

| CCA | Goodput mean | p95 mean | Notes |
|-----|-------------:|---------:|-------|
| CUBIC | 5.47 | 124.8 | Collapses under mobility |
| BBRv3approx | 70.88 | 138.8 | Reference |
| **LeoAware v3.1** | **68.98** | **133.6** | **p95 under BBR; +2.5 Mbps vs v3** |
| LeoAware v3 (prior) | 66.44 | 133.2 | Accepted under same gate |

### Suite seed 13 snapshot (not the optimize target)
...
| leo_fast_ho + hints | LeoAware | 71.67 | 144.1 |

v3.1 levers: path-aware BDP sizing, soft min_rtt age, post-hop max-filter reclaim, ASCENT freeze next_capacity peek. Log: `docs/experiment_log.md`. Design: `docs/leoaware_v2_design.md`, `docs/traces_and_ascent.md`.
