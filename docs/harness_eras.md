...
| **`wetlinks_v1` CSV** | 5x90s hold-expanded WetLinks `net_iperf`+`net_ping` slices | same absolute bars unless re-derived | v3.11 measured-CSV era. **Not** product default. Do not mix with `starlink_v1` Crest 82.07/76.26. |
| `zhao_zenodo23` CSV | 5 calendar-quantile Zhao/Pan IRTT+iperf3 Cubic slices | same bars; cubic-gp oracle is a **lower bound** | v3.12 research ingest. **Not** product. Do not use for dual-gate ACCEPT. |
| **`leocc_v1` CSV** | 5 catalog-quantile LeoCC downlink 90s UDP-sat + ICMP OWD | same absolute bars; RTT = 2xOWD | v3.13 measured-CSV era. **Not** product default. Do not mix with `wetlinks_v1` / `zhao_zenodo23` / Crest 82.07/76.26. |
